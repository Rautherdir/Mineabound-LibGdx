...
import com.tycoon177.mineabound.utils.Rand;
...
public class Chunk {
	...
	public void generateChunk() {
		PerlinNoiseGenerator noiseMap = new PerlinNoiseGenerator(12345);
		System.out.println(Rand.d(10000));
		...
		for(int i = 0; i < 15; i++) {
			currentHeight += noiseMap.improvedNoise(offset - i/15.0, 0, 0) * 17.3;
			System.out.println(noiseMap.improvedNoise(offset + i/16.0, 0, 0));
			currentHeight = MathUtils.clamp(currentHeight, 0, HEIGHT - 1);
			buffer = currentHeight;
			block[i][currentHeight] = new Block(BlockType.GRASS);
			currentHeight = 20;
			currentHeight += noiseMap.improvedNoise(offset - i/15.0, 0, 0) * 10;
			System.out.println(noiseMap.improvedNoise(offset + i/16.0, 0, 0));
			currentHeight = MathUtils.clamp(currentHeight, 0, HEIGHT - 1);
			block[i][currentHeight] = new Block(BlockType.STONE);
			for(int n = currentHeight+1; n < buffer; n++) {
				block[i][n] = new Block(BlockType.DIRT);
			}
			currentHeight = 30;
		}
/*
